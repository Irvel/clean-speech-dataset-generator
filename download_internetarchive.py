...
"""
CATEGORIES_WEIGHTS = [0.267, 0.165, 0.076, 0.38, 0.085, 0.03]
...
def fetch_items_in_query(search_query, num_items):
    ...
    for item in internetarchive.search_items(query=search_query):
        if len(items) < num_items:
            item = internetarchive.get_item(item["identifier"])
            if item:
                items.append(item)
        else:
            break
...
def fetch_total_n_items(num_items, uniform_distribution=False):
    ...
    how_many_of_each_cat = [int(w * num_items) for w in categories_weights]
...
def download_n_files(destination_dir, num_files=NUM_FILES):
    ...
    for item in items:
        ...
        if file_name:
            try:
                logger.info(f"Fetching info for {num_files} internetarchive items...")
...
if __name__ == '__main__':
    downloaded_bytes = 0
    while downloaded_bytes < 10000000000:
